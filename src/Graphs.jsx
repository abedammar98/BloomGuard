...
/* ---------- helpers (hoisted so HoverPanel can use them) ---------- */
function fmt(v) {
...
function zoneMessage(row) {
  if (!row) return "";
  if (row.message) return row.message; // server-provided message preferred
  const z = row.zone;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Debug: print the full advisory table to console (disable once happy)
        console.table(
          (j?.rows || []).map((r) => ({
            date: r.date,
            CDI: r.CDI,
            ...
            temp_c: r.temp_c,
            ph: r.ph,
            oxygen_mgL: r.oxygen_mgL,
            chlorophyll_a_ugL: r.chlorophyll_a_ugL,
            nitrate_mgL: r.nitrate_mgL,
            nitrite_mgL: r.nitrite_mgL,
            turbidity_NTU: r.turbidity_NTU,
            flagged_drivers: Array.isArray(r.flagged_drivers)
              ? r.flagged_drivers.join(",")
              : r.flagged_drivers,
          }))
        );
        ...
        const adv = (j?.rows || []).map((r) => {
          ...
          // coerce driver columns to numbers/null so HoverPanel shows them
...
export default function Graphs() {
  ...
  // quick map: dateLabel -> index (more reliable than activeTooltipIndex)
